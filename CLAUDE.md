# CLAUDE.md
Persistent Claude Code instructions for **Project Atlas**.
...
8. Holdouts are locked. Do not expose locked holdout labels or records to simulation agents; `src/atlas/judge/holdouts.py` handles runtime gating and `.claude/settings.json` denies Claude Code reads of locked holdout files.
...
11. Run `make safety-scan` before demos, commits, generated-text changes, API-response changes, fixture changes, or UI-copy changes. Claude Code hooks also run targeted scans after relevant writes and before session stop.
...
- `.claude/settings.json` owns Claude Code permissions and hooks.
- `.claude/hooks/` runs targeted safety scans after relevant edits and before session stop.
- `.claude/skills/` holds derived, lazy-loaded operational knowledge from the Bible.
- `.claude/agents/` contains builder subagents only; they assist development and never participate in a Project Atlas round.
...
- `builder_subagents` are Claude Code helpers under `.claude/agents/`. They assist the human developer only and never participate in a Project Atlas round.
...
## Build order
1. Repo skeleton, configs, `.claude/settings.json`, `.claude/hooks/`, `.claude/skills/`, `.mcp.json`, and `scripts/safety_scan.py`.
